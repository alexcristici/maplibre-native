...
bool bindFloat(const FloatBinder& binder,
               const mbgl::PossiblyEvaluatedPropertyValue<float>& current,
               gfx::VertexAttributeArray& attrs, std::string attributeName) {
    ...
    if (auto& attr = attrs.getOrAdd(std::move(attributeName))) {
        for (std::size_t i = 0; i < count; ++i) {
            if (count) {
                ...
                attr->set(0, std::get<0>(binder.uniformValue(current)));
            }
        }
        return true;
    }
...
bool bindFloat(const std::unique_ptr<FloatBinder>& binder,
    ...
    return binder && bindFloat(*binder, current, attrs, std::move(attributeName));
}

bool bindFloatPair(const FloatPairBinder& binder,
                   [[maybe_unused]] const mbgl::PossiblyEvaluatedPropertyValue<FloatPair>& current,
                   gfx::VertexAttributeArray& attrs, std::string attributeName) {
    const auto count = binder.getVertexCount();
...
bool bindFloatPair(const std::unique_ptr<FloatPairBinder>& binder,
                   const mbgl::PossiblyEvaluatedPropertyValue<FloatPair>& current,
                   gfx::VertexAttributeArray& attrs,
                   std::string attributeName) {
    return binder && bindFloatPair(*binder, current, attrs, std::move(attributeName));
}

bool bindColor(const ColorBinder& binder,
               const mbgl::PossiblyEvaluatedPropertyValue<Color>& current,
               gfx::VertexAttributeArray& attrs, std::string attributeName) {
    const auto count = binder.getVertexCount();
    // check that vertexVector.elements() == sum(segments.vertexLength)
    if (auto& attr = attrs.getOrAdd(std::move(attributeName))) {
        if (count) {
            ...
            const Color& color = std::get<0>(binder.uniformValue(current));
            attr->set(0, util::cast<float>(color.toArray()));
        }
        return true;
    }
    return false;
...
bool bindColor(const std::unique_ptr<ColorBinder>& binder,
               ...
               gfx::VertexAttributeArray& attrs,
               std::string attributeName) {
    return binder && bindColor(*binder, current, attrs, std::move(attributeName));
}

template <std::size_t N>
bool bindImage(const ImageBinder& binder,
               const mbgl::PossiblyEvaluatedPropertyValue<mbgl::Faded<mbgl::style::expression::Image>>& current,
               gfx::VertexAttributeArray& attrs, std::string attributeName) {
    const auto count = binder.getVertexCount();
    // check that vertexVector.elements() == sum(segments.vertexLength)
    if (auto& attr = attrs.getOrAdd(std::move(attributeName))) {
        if (count) {
            for (std::size_t i = 0; i < count; ++i) {
                const auto& value = std::get<0>(binder.getVertexValue(i)).a1;
                std::array<int32_t,4> slice;
                std::copy(std::next(value.begin(),N*4), std::prev(value.end(),4-N*4), slice.begin());
                attr->set(i, util::cast<int32_t>(slice));
            }
        } else {
            const std::array<unsigned short, 4>& value = std::get<N>(binder.uniformValue(current));
            attr->set(0, util::cast<int32_t>(value));
        }
        return true;
    }
    return false;
...
bool bindImage(const std::unique_ptr<ImageBinder>& binder,
               ...
               gfx::VertexAttributeArray& attrs,
               std::string attributeName) {
    return binder && bindImage<N>(*binder, current, attrs, std::move(attributeName));
}
...
void RenderFillLayer::update(gfx::ShaderRegistry& shaders,
    ...
    for (const RenderTile& tile : *renderTiles) {
        ...
        if (unevaluated.get<FillPattern>().isUndefined()) {
            ...
            bindColor(paintPropertyBinders.get<FillColor>(), evaluated.get<FillColor>(), fillVertexAttrs, "a_color");
            bindFloat(paintPropertyBinders.get<FillOpacity>(), evaluated.get<FillOpacity>(), fillVertexAttrs, "a_opacity");

            if (doOutline) {
                bindColor(paintPropertyBinders.get<FillOutlineColor>(), evaluated.get<FillOutlineColor>(), outlineVertexAttrs, "a_outline_color");
                bindFloat(paintPropertyBinders.get<FillOpacity>(), evaluated.get<FillOpacity>(), outlineVertexAttrs, "a_opacity");
            }
            ...
            bindFloat(paintPropertyBinders.get<FillOpacity>(), evaluated.get<FillOpacity>(), fillVertexAttrs, "a_opacity");
            bindImage<0>(paintPropertyBinders.get<FillPattern>(), evaluated.get<FillPattern>(), fillVertexAttrs, "a_pattern_from");
            bindImage<1>(paintPropertyBinders.get<FillPattern>(), evaluated.get<FillPattern>(), fillVertexAttrs, "a_pattern_to");

            if (doOutline) {
                bindFloat(paintPropertyBinders.get<FillOpacity>(), evaluated.get<FillOpacity>(), outlineVertexAttrs, "a_opacity");
                bindImage<0>(paintPropertyBinders.get<FillPattern>(), evaluated.get<FillPattern>(), outlineVertexAttrs, "a_pattern_from");
                bindImage<1>(paintPropertyBinders.get<FillPattern>(), evaluated.get<FillPattern>(), outlineVertexAttrs, "a_pattern_to");
            }
